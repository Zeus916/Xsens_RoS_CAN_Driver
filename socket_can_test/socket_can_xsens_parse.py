...
def format_data(data):
    # print(data)
    # print([hex(byte)[2:] for byte in data])
...
def listen_cmd(args):
    ...
    while True:
        try:
            ...
            parse(cob_id, data)
            #print(hex(cob_id), data)
        except KeyboardInterrupt:
            print("Exiting..")
            break

def parse(cob_id,data):
    k = None
    if hex(cob_id)=='0x21':
        print("Quaternion")
        k = [i*((2**15)-1)**-1 for i in unpack('>hhhh', data)]
        
    elif hex(cob_id)=='0x34':
        print("Acceleration")
        k = [i*2**-8 for i in unpack('>hhh', data)]
    
    elif hex(cob_id)=='0x32':
        print("Rate of Turn")
        k = [i*2**-9 for i in unpack('>hhh', data)]

    elif hex(cob_id)=='0x41':
        print("Mag field")
        k = [i*2**-10 for i in unpack('>hhh', data)]

    elif hex(cob_id)=='0x71':
        print("Lat-lon")
        k = [a * b for a, b in zip((2**-24,2**-23),unpack('>ll',data))]
    print(k)
def parse_args():
